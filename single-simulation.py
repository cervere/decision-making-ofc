...
SP = np.zeros((num_sessions, num_trials))
SDE = np.zeros((num_sessions, num_trials))
SWW = np.zeros((NUM_CHANNELS, num_sessions, num_trials))
DT = {"cog" : np.zeros((num_sessions, num_trials)), "mot" : np.zeros((num_sessions, num_trials)), "mofc" : np.zeros((num_sessions, num_trials))}
MVS = np.zeros((num_sessions, num_trials))
for k in range(num_sessions):
    np.random.shuffle(C)
    np.random.shuffle(M)
    exLoop.resetBrain()
    P, R, MV = [], [], []
    dts_cog, dts_mot, dts_mofc = [], [], []

    sample_history = []
    DE = []
    TWW = np.zeros((num_trials, NUM_CHANNELS))
    exLoop.cogchoiceHistory = np.zeros(num_trials)
    exLoop.rewardHistory = np.zeros(num_trials)
    for j in range(num_trials):
        ...
        if mot_choice > -1:
            dts_cog.append(exLoop.lastCogTime)
            dts_mofc.append(exLoop.lastmOFCTime)
            dts_mot.append(exLoop.lastMotTime)
            c1, c2 = cues_cog
            m1, m2 = cues_mot
        ...
        MV.append(np.mean(exLoop.cues_value[:3]))

        exLoop.resetInputs()
        iteratefor(duration-settle,duration)
        P.append(perf)
        DE.append(exLoop.value_diff)
        TWW[j] = Wmain
...
saveResults(SP, SWW, SDE, DT, sample_history, MVS)
# -----------------------------------------------------------------------------
from display import *

if 1:
    display_ctx(sample_history, 3.0, "figure-1.svg")
    display_performance([SWW,SP,SDE], num_trials)
if 0:
